...
class DLPackTensor:
    ...
    def _from_dlpack_fallback(capsule: Any) -> np.ndarray:
        ...
        strides = tuple(dl_tensor.strides[i] * dl_tensor.dtype_bits // 8 
        ...
        arr = np.ndarray(
            shape=shape,
            dtype=dtype,
            buffer=(ctypes.c_char * (dl_tensor.numel * dl_tensor.dtype_bits // 8)).from_address(dl_tensor.data),
            strides=strides,
        )
...
class PybindJITModule:
    ...
    def _load_kernel(self):
        ...
        if runtime._native_module:
            return runtime._native_module.CompiledKernel(
                str(self._lib_path),
                "run_kernel"
            )
        return None
...
class PybindJITEngine:
    ...
    def _build_command(
        ...
        cmd = [
            compiler, "-std=c++17", "-O3", "-march=native",
            "-shared", "-fPIC", str(cpp_path), "-o", str(lib_path),
        ]
        if openmp:
            cmd.append("-fopenmp")
        return cmd
