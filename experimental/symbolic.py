...
class SymbolicShapeInference:
    ...
    def infer_elementwise_shape(self, lhs_shape: SymbolicShape, 
                                 rhs_shape: SymbolicShape) -> SymbolicShape:
    ...
    def infer_matmul_shape(self, a_shape: SymbolicShape, 
                           b_shape: SymbolicShape) -> SymbolicShape:
        ...
        batch_shape = self.infer_elementwise_shape(
            SymbolicShape(batch_a),
            SymbolicShape(batch_b)
        )
    ...
    def infer_reduce_shape(self, input_shape: SymbolicShape, 
                           axes: Tuple[int, ...],
                           keepdims: bool = False) -> SymbolicShape:
    ...
    def infer_reshape_shape(self, input_shape: SymbolicShape,
                            target_dims: List[SymbolicDim]) -> SymbolicShape:
        ...
        if unknown_idx >= 0:
            ...
            target_dims = target_dims[:unknown_idx] + [inferred_dim] + target_dims[unknown_idx + 1:]
    ...
    def infer_broadcast_shape(self, input_shape: SymbolicShape,
                              target_shape: SymbolicShape) -> SymbolicShape:
        """Infer shape for broadcast_to operation."""
        return self.infer_elementwise_shape(input_shape, target_shape)
    
    def infer_transpose_shape(self, input_shape: SymbolicShape,
                              perm: Optional[Tuple[int, ...]] = None) -> SymbolicShape:
    ...
    def infer_slice_shape(self, input_shape: SymbolicShape,
                          starts: Tuple[int, ...],
                          ends: Tuple[int, ...],
                          steps: Optional[Tuple[int, ...]] = None) -> SymbolicShape:
        """Infer shape for slice operation."""
        if steps is None:
            steps = (1,) * len(starts)
        
        result_dims = []
        for i, (dim, start, end, step) in enumerate(
            zip(input_shape.dims, starts, ends, steps)
        ):
            if dim.is_constant():
                d = dim.evaluate({})
                length = max(0, (min(end, d) - start + step - 1) // step if step > 0 
    ...
    def validate_shape(self, shape: SymbolicShape, 
                       expected: SymbolicShape) -> bool:
