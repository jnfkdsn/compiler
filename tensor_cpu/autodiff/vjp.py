...
def add_binary(graph: Graph, op: OpType, lhs: Node, rhs: Node, name: str) -> Node:
    ...
    return graph.add_node(
        op_type=op, name=name, inputs=[lhs.id, rhs.id], shape=out_shape, dtype=out_dtype
    )
...
def reduce_to_shape(
    graph: Graph, grad: Node, target_shape: tuple[int, ...], name_prefix: str
) -> Node:
...
def _eager_sum(
    g: np.ndarray, x: np.ndarray, o: np.ndarray, axes: tuple, keepdims: bool
) -> np.ndarray:
